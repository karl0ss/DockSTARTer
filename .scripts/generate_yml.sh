            ...
            if [[ -f ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.yml ]]; then
                if [[ ${ARCH} == "aarch64" ]]; then
                    if [[ -f ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.arm64.yml ]]; then
                        echo "${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.arm64.yml \\" >> "${RUNFILE}"
                    elif [[ -f ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.armhf.yml ]]; then
                        echo "${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.armhf.yml \\" >> "${RUNFILE}"
                        info "Missing arm64 option for ${APPNAME} (may not be available) falling back on armhf."
                    else
                        error "Failed to find ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.arm64.yml file or ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.armhf.yml file."
                        continue
                    fi
                fi
                if [[ ${ARCH} == "armv7l" ]]; then
                    if [[ -f ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.armhf.yml ]]; then
                        ...
                        error "Failed to find ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.armhf.yml file."
                        continue
                    fi
                fi
                if [[ -z ${APPNETMODE} ]] || [[ ${APPNETMODE} == "bridge" ]]; then
                    if [[ -f ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.ports.yml ]]; then
                        echo "${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.ports.yml \\" >> "${RUNFILE}"
                        info "${APPNAME}_NETWORK_MODE supports port mapping. Ports will be included."
                    else
                        warning "Failed to find ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.ports.yml file."
                    fi
                fi
                if [[ -n ${APPNETMODE} ]]; then
                    if [[ -f ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.netmode.yml ]]; then
                        echo "${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.netmode.yml \\" >> "${RUNFILE}"
                        info "${APPNAME}_NETWORK_MODE is set to ${APPNETMODE}."
                    else
                        warning "Failed to find ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.netmode.yml file."
                    fi
                fi
                echo "${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.yml \\" >> "${RUNFILE}"
                info "All configurations for ${APPNAME} are included."
            else
                warning "Failed to find ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/${FILENAME}.yml file."
            fi
        else
            error "Failed to find ${SCRIPTPATH}/compose/.apps/${TESTFOLDER}/${FILENAME}/ directory."
        fi
